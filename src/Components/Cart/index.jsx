...
const Cart = () => {
    ...
    useEffect(() => {
       setProductsLength(
        cartItems.reduce((previous,current) => previous + current.Cantidad_disponible,0)
       )  
    }, [cartItems]);

    const total = cartItems.reduce((previous,current)=> previous + current.Cantidad_disponible * current.Precio, 0 );
    ...
    return (
        <div className={styles.cartContainer}> 
            ...
            {cartItems.length && cartOpen &&(
                   <div className={styles.cart}>    
                      <h2>Tu carro</h2>
                      {cartItems.length ===  0 ? (
                      <p className={styles.cartVacio}>Tu carro de compra vacio</p>
                      ): (
